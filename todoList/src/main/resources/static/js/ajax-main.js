...
/* fetch() API : 비동기 요청을 수행하는 최신 JS API 중 하나
  - Promise(객체) : 비동기 작업의 결과를 처리하는 방법
    ...
    - Promise 객체는 세 가지의 상태를 가짐(PromiseState)
      - Pending (대기 중) : 비동기 작업이 완료되지 않은 상태
      - Fulfilled(이행됨) : 비동기 작업이 성공적으로 완료된 상태 > PromiseResult로 결과값을 가져옴
...
function getTotalCount() {
  ...
  fetch("/ajax/totalCount") // 서버로 "/ajax/totalCount"로 GET 요청
    // 첫 번째 then 구문 : fetch 구문에서 서버로 보낸 요청에 대해 응답을 처리하는 역할
    // 서버에서 응답을 받으면 > 이 응답(response, 매개변수)을 텍스트 형식으로 변환하는 콜백함수
    .then((response) => {
    ...
    .then((result) => {
      // id가 totalCount인 sapn 태그의 내용으로 result 값 삽입(화면에 출력) 
