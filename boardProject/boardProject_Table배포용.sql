...
CREATE TABLE "MEMBER" (
	...
	-- 삭제 여부 : 회원 탈퇴 시 DELETE 요청을 전송하는 것이 아닌 > Y/N 여부만 바꿔두었다가 > 주기적으로 탈퇴된 회원 삭제
...
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		AND R2.OPEN_MEMBER = 1), -- 사용자(나)가 채팅방을 개설했을 경우
		R.PARTICIPANT,
		R.OPEN_MEMBER
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
...
FROM "MEMBER"
WHERE (MEMBER_EMAIL LIKE '%이%' OR MEMBER_NICKNAME LIKE '%이%')
AND MEMBER_DEL_FL = 'N'
AND MEMBER_NO != 1;

-- 채팅방 번호 체크
-- 존재하지 않는 경우 NULL이 넘어가므로 NVL 처리를 해주어야 함
SELECT NVL(SUM(CHATTING_ROOM_NO), 0) CHATTING_ROOM_NO
FROM CHATTING_ROOM
WHERE (OPEN_MEMBER = 1 AND PARTICIPANT = 3)
OR (OPEN_MEMBER = 3 AND PARTICIPANT = 1)
