...
// - 웹소켓 동작 시 웹소켓을 통해 수행할 동작의 구문을 작성하는 클래스

@Component // Bean 등록
@Slf4j
public class TestWebsocketHandler extends TextWebSocketHandler {
	
	// WebSocketSession : 클라이언트-서버 간 양방향 통신을 담당하는 객체
	// SessionHandshakeInterceptor가 가로챈, 연결된 클라이언트의 HttpSession을 가지고 있음
	
	// 동기화된 Set 객체 생성
	private Set<WebSocketSession> sessions = Collections.synchronizedSet(new HashSet<>());
	// -> 여러 스레드가 동작하는 환경에서 하나의 컬렉션에 여러 스레드가 접근하여 의도치 않은 문제가 발생할 수 있음
	// -> 이를 방지하고자 동기화를 진행해 스레드가 순서대로 동작(접근)하여 꼬이지 않게 함 

	// 클라이언트와 연결이 완료되고, 통신할 준비가 되면 실행
	@Override
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		// 연결된 클라이언트의 WebSocketSession 정보를 Set에 추가 -> 웹 소켓에 연결된 클라이언트 정보를 모아두기 위해
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		// 웹 소켓 연결이 끊긴 클라이언트의 정보를 Set 객체에서 제거
...
TextWebSocketHandler : WebSocketHandler 인터페이스를 상속받아 구현한 텍스트 메세지 전용 웹소켓 핸들러 클래스
  ...
  - 클라이언트로부터 텍스트만 담겨져 있는 메세지를 받았을때 실행
