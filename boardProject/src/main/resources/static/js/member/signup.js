...
// 2) 이메일이 입력(input 이벤트 발생) 될 때 마다 유효성 검사 수행
memberEmail.addEventListener("input", e => {
    ...
    // 비동기(ajax) > fetch API 사용
    fetch("/member/checkEmail?memberEmail=" + inputEmail)
    .then(resp => resp.text())
    .then(count => {
        // count : 1이면 > 중복을 의미, 0이면 > 중복이 아님을 의미
        if(count == 1) {
        ...
        checkObj.memberEmail = true; // 유효한 이메일을 의미
    }).catch( error => {
        // fetch API를 이용한 비동기 요청 수행 중 발생한 예외 처리
        console.log(error);
    })
});
...
sendAuthKeyBtn.addEventListener("click", () => {
    ...
    // *************************************
    // 비동기로 서버에서 메일보내기 
    // Java Mail Sender : 자바에서 메일을 보낼 수 있게 해주는 모듈/라이브러리
    // 구글 SMTP 서버 + 자바, 2단계 인증 + APP KEY(앱 비밀번호) 발급받아서 사용해야
    fetch("/email/signup", {
        method : "POST",
...
//   같으면 1, 아니면 0반환
// 단, 타이머가 00:00초가 아닐 경우에만 수행 > 인 관련증 유효 시간
...
checkAuthKeyBtn.addEventListener("click", () => {
    ...
    .then(result => {
        ...
        // 일치할 때 > 타이머 멈춰주고 > 그 자리에 "인증되었습니다." 출력
        clearInterval(authTimer); // 타이머 멈춤
        authKeyMessage.innerText = "인증되었습니다.";
        authKeyMessage.classList.remove("error");
        authKeyMessage.classList.add("confirm");

        checkObj.authKey = true; // 인증 번호 검사 여부를 true로 변경
    })
