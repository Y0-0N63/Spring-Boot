...
// 1) 로그인한 회원 번호 준비
//     --> session에서 얻어오기(session은 서버에서 관리하기때문에 JS에서 바로 얻어올 방법 없음)
// 2) 현재 게시글 번호 준비
// 3) 좋아요 여부 준비
...
document.querySelector("#boardLike").addEventListener("click", e => {
  ...
  // 2. 로그인 상태가 아닌 경우 > 동작하면 안 됨
  if(loginMemberNo == null) {
    alert("로그인 후 이용해주세요.");
    return;
  }

  // 3. 로그인 상태라면 > 준비된 3개의 변수를 JS 객체로 생성 (JSON으로 변환할 예정)
  const obj = {
  ...
  // 4. 좋아요를 INSERT하거나 DELETE하는 비동기 요청 보내기 (fetch를 이용, obj를 JSON형태로 변환해야)
  fetch("/board/like", {
    method : "POST",
  ...
  .then(count => {
    ...
    // 5. likeCheck 값을 0 <-> 1 변환 > 클릭될 때마다 INSERT or DELETE 번갈아서 수행할 수 있도록 (비동기 요청을 연속으로 보낼 때의 처리)
    likeCheck = likeCheck == 0 ? 1 : 0;

    // 6. 화면상의 하트 아이콘을 채우기<->비우기
...
if(updateBtn != null) { // 수정 버튼 존재 시
    updateBtn.addEventListener("click", () => {
        ...
        // location.pathname : 현재 경로, .replace() : 'board'를 'editBoard'로 교체
